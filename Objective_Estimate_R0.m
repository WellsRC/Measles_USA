...
R0=interp1(County_Data.beta_j',County_Data.R_0',beta_j)';
...
L_RE=Cases_Data.*log(gampdf(Reff(:),5.3110,1.6362)); 
L_RE=L_RE(Cases_Data>0);
...
if((median(R0)<=18.1 && median(R0)>=10.7) && (~isinf(-mean(L_RE(:))-mean(L_Control(:)))))
    L_R0=0;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if(~isinf(-L_R0 -mean(L_Unknown(:)+L_Known(:))-mean(L_RE(:))-mean(L_Control(:))))
    ...
    J=-mean(L_Unknown(:)+L_Known(:))-mean(L_RE(:))-mean(L_Weekly_2025(:)) -mean(L_Control(:)) -L_R0 ; 
else
    J=inf;
end
